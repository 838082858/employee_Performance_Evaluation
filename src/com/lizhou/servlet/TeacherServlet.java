...
public class TeacherServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String method = request.getParameter("method");
		//请求分发
		if("TeacherList".equalsIgnoreCase(method)){ //获取所有领导数据
			teacherList(request, response);
		} else if("AddTeacher".equalsIgnoreCase(method)){ //添加领导
			addTeacher(request, response);
		} else if("DeleteTeacher".equalsIgnoreCase(method)){ //删除领导
			deleteTeacher(request, response);
		} else if("EditTeacher".equalsIgnoreCase(method)){ //修改领导信息
			editTeacher(request, response);
		} else if("GetTeacher".equalsIgnoreCase(method)){ //获取某个领导信息
			getTeacher(request, response);
		} else if("GetExamClazz".equalsIgnoreCase(method)){ //获取某次考核领导带领的工作小组
			getExamClazz(request, response);
		} else if("GetExamCourse".equalsIgnoreCase(method)){ //获取某次考核领导负责的考核项目
...
//		int clazzId = request.getParameter("clazzid") == null ? 0 : Integer.parseInt(request.getParameter("clazzid").toString());
...
//		if(userType == 3){
//			//如果是老师，只能查看自己的管理的绩效信息
////			Student currentUser = (Student)request.getSession().getAttribute("user");
//			studentId = user.getId();
//		}
//		leave.setClazzId(studentId);
